...
public class DragImageView extends ImageView {
	...
	private int save_x,save_y;//原始点击的位置
	
	private int strechFlag;
...
//	private ScaleAnimation scaleAnimation;// 缩放动画
	 ...
	 * 控制底部的布局
	 */
	public static final int SHOW_BOTTOM = 0x11;
	/**禁止viewpager滚动*/
	public static final int FORBID_SCROLL = 0x12;
	/**允许viewpager滚动*/
	public static final int ALLOW_SCROLL = 0x13;
...
//	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			mode = MODE.NONE;
			int distance_x = Math.abs(save_x - current_x);
			int distance_y = Math.abs(save_y- current_y);
			if ( distance_x < 20 && distance_y < 20 && strechFlag != 2) {
				handler.sendEmptyMessage(SHOW_BOTTOM);
			}
			strechFlag = 1;
			break;
	...
	void onTouchDown(MotionEvent event) {
		mode = MODE.DRAG;
		handler.sendEmptyMessage(ALLOW_SCROLL);
	...
	void onPointerDown(MotionEvent event) {
		...
		if (event.getPointerCount() > 1) {
			handler.sendEmptyMessage(FORBID_SCROLL);
		}
	}
	...
	public void doScaleAnim() {
		myAsyncTask = new MyAsyncTask(screen_W, 
//				this.screen_W,
				this.getWidth(),
//				this.screen_H);
				this.getHeight());
		myAsyncTask.setLTRB(this.getLeft(), this.getTop(),
//				this.screen_W,
				this.getRight(),
				this.screen_H);//需要图片高度等于屏幕高
//				this.getBottom());
		myAsyncTask.execute();
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		public MyAsyncTask(int screen_W, int current_Width, int current_Height) {
			...
			Log.d(TAG, "current_height"+current_Height);
		}
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
                ...
                Log.e("jj", "top="+top+",bottom="+bottom+",left="+left+",right="+right+"none"+current_Height);
